...
class inputs:
    ...
    def changer(self):
        ...
        row_index = 0  # Row index for iterating through self.df
        i = 0
        while i + 2 < len(self.data):  # Ensure we don't go out of bounds
            # Take 3 bits from self.data for R, G, B
            red_bit = self.data[i]
            green_bit = self.data[i + 1] if i + 1 < len(self.data) else None
            blue_bit = self.data[i + 2] if i + 2 < len(self.data) else None
            
            # Ensure we are not out of bounds and we have an existing row to append to
            if row_index < len(self.df):
                if red_bit:
                    self.df.at[row_index, 'R'] += red_bit
                if green_bit:
                    self.df.at[row_index, 'G'] += green_bit
                if blue_bit:
                    self.df.at[row_index, 'B'] += blue_bit
            
            i += 3  # Move to the next set of bits for the next row
